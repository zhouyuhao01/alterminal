package aiterminal.android.chdmc.com.aiterminal.component;
...
public class BottomBar extends View {
    ...
    public interface OnFragmentChangedListener {
        ...
        void onFragmentChanged(int whichFragment);
    }
...
//            paint.setAntiAlias(false);
            ...
            for (int i = 0; i < itemCount; i++) {
                float radius = iconHeight;
                if (i == currentCheckedIndex) {
                    radius = getHeight() * 0.9f / 2;
                } else {
                    radius = getHeight() * 0.7f / 2;
                }
                float circleX = getWidth() * (2 * i + 1) / (itemCount*2)  ;
                float circleY = getHeight() / 2;
                paint.setColor(0xff249cd3);
            ...
            for (int i = 0; i < itemCount; i ++) {
                ...
                float circleX = getWidth() * (2 * i + 1) / (itemCount*2) - (title.length() * paint.getTextSize() / 2) ;

                Paint.FontMetrics fontMetrics = paint.getFontMetrics();
                float distance = (fontMetrics.bottom - fontMetrics.top)/2 - fontMetrics.bottom;
                float baseline = getHeight() / 2 + distance;
                canvas.drawText(title, circleX, baseline, paint);
            }
        }
    }
    ...
    protected void switchFragment(int whichFragment) {
        ...
        if (this.onFragmentChangedListener != null) {
            this.onFragmentChangedListener.onFragmentChanged(whichFragment);
        }
